...
public interface OffreMapper {
    ...
    @Insert("""
        INSERT INTO offre (poste, contrat, localisation, salaire, date_publication, entreprise_id, status)
        VALUES (#{poste}, #{contrat}, #{localisation}, #{salaire}, #{datePublication}, #{entrepriseId}, #{status})
    """)
    ...
    @Update("""
        UPDATE offre 
        SET poste = #{poste}, contrat = #{contrat}, localisation = #{localisation},
            salaire = #{salaire}, date_publication = #{datePublication},
            entreprise_id = #{entrepriseId}, status = #{status}
        WHERE id = #{id}
    """)
    ...
    @Select("SELECT * FROM offre ORDER BY date_publication DESC LIMIT #{limit} OFFSET #{offset}")
